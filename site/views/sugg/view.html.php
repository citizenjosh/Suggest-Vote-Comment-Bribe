...
class SuggestionViewsugg extends JView
{
   ...
   function display($tpl = null)
   {
      ...
      $db=JFactory::getDBO();
      $cids.='0';
      $db->setQuery('SELECT * FROM #__suggestion_comment where SID='.$item->id.' and (published=1 or (UID='.$user->id.' and UID!=0) or id in('.$cids.')) order by id');
      $comments = $db->loadObjectlist();
      $this->assignRef('comments', $comments);
      $db->setQuery('SELECT * FROM #__suggestion_vote where SID='.$item->id.' and (published=1 or (UID='.$user->id.' and UID!=0)) order by id');
      $votes = $db->loadObjectlist();
      $this->assignRef('votes', $votes);
//      $db->setQuery('SELECT sum(value) as sum FROM #__suggestion_vote where SID='.$item->id.' and published=1 group by SID');
